...
def main(argv):
    ...
    ap.add_argument("--prefer-newer", action="store_true", help="When hashes differ, keep the newer file")
...
"""
    ...
    # Auto-append telemetry event (only if telemetry already has at least one entry)
    try:
        sys.path.insert(0, str(dirs["scripts"]))
        from telemetry import append_event as _append_event
        tel_path = PROJECT_ROOT / "thread" / "telemetry.jsonl"
        tel_lines = 0
        if tel_path.exists():
            with tel_path.open("r", encoding="utf-8", errors="ignore") as _f:
                tel_lines = sum(1 for _ in _f if _.strip())
        if _append_event and tel_lines > 0:
            _append_event({
