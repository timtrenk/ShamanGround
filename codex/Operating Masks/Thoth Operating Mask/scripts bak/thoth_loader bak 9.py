...
def copy_into(src: Path, dst: Path, verbose=False, dry=False, trace=False, theater=False):
    if not src.exists():
        if verbose:
...
def main(argv):
    ...
    print(f"mode: {'DRY-RUN' if dry else 'WRITE'}  verbose:{verbose}  trace:{trace}  theater:{theater}")
    ...
    with StepTimer("stage", theater=theater):
        ...
        for i,(src,dst) in enumerate(manifest):
            ...
            created, src_h, dst_h, _ = copy_into(src, dst, verbose=True, dry=dry, trace=trace, theater=theater)
            if created: staged += 1
            else: kept += 1
